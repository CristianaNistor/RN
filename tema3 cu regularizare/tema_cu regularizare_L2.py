...
batch_size = 32
lr = 0.1
lambda_reg = 1e-4 # coef de regularizare L2
dropout_rate = 0.2
...
for epoch in range(epochs):
    ...
    for i in range(0, len(X_train), batch_size):
        ...
        # aplica dropout si scaleaza activarile pt a pastra magnitudinea asteptata
        ...
        # np.arange(len(y_batch)) creeaza o secventa de indici pentru fiecare rand din batch
        ...
        dW2 = a1.T.dot(probs) + lambda_reg * W2
        # dW2 = dL/dw2 = a1.T dL/dz2 = a1.T probs si mai adaug penalizarea pentru regularizare lambda_reg*w2
        ...
        dW1 = X_batch.T.dot(dz1) + lambda_reg * W1 # dL/dw1 = x.T x dL/dz1 = x.T x dz1; adaug si aici penalizarea
...
pd.DataFrame({"ID": np.arange(len(preds)), "target": preds}).to_csv("submission_with_L2.csv", index=False)
